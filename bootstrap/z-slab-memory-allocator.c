...
#define GET_NEW(ctx,allocator) __z_slab_memory_allocator_new(ctx,allocator)
#define INIT_EXISTS
#define init z_slab_memory_allocator_init
#define new z_slab_memory_allocator_new

int z_slab_memory_allocator_type_id = -1;

static Self *__z_slab_memory_allocator_new(struct zco_context_t *ctx, ZMemoryAllocator *allocator)
{
	Self *self = NULL;
	if (allocator) {
		ZObjectTracker *object_tracker = z_memory_allocator_get_object_tracker(allocator);
		if (object_tracker) {
			self = (Self *) z_object_tracker_create(object_tracker, z_slab_memory_allocator_type_id);
...
static void z_slab_memory_allocator_init(Self *self);
static void  z_slab_memory_allocator_dispose(ZObject *object);
static void *  z_slab_memory_allocator_allocate(ZMemoryAllocator *allocator,int size);
static void *  z_slab_memory_allocator_allocate_aligned(ZMemoryAllocator *allocator,int size,int alignment);
static int  z_slab_memory_allocator_try_resize(ZMemoryAllocator *allocator,void *block,int new_size);
static void *  z_slab_memory_allocator_resize(ZMemoryAllocator *allocator,void *block,int new_size);
static void  z_slab_memory_allocator_deallocate(ZMemoryAllocator *allocator,void *block);
static void z_slab_memory_allocator_class_destroy(ZObjectGlobal *gbl);
...
ZSlabMemoryAllocatorGlobal * z_slab_memory_allocator_get_type(struct zco_context_t *ctx)
{
	...
	if (z_slab_memory_allocator_type_id != -1) {
		global_ptr = zco_get_ctx_type(ctx, z_slab_memory_allocator_type_id);
	}
	if (!global_ptr || !*global_ptr) {
		struct ZSlabMemoryAllocatorGlobal *global = (ZSlabMemoryAllocatorGlobal *) malloc(sizeof(struct ZSlabMemoryAllocatorGlobal));
		global->common.ctx = ctx;
		global->_class = malloc(sizeof(struct ZSlabMemoryAllocatorClass));
		memset(CLASS_FROM_GLOBAL(global), 0, sizeof(struct ZSlabMemoryAllocatorClass));
		global->common.name = "ZSlabMemoryAllocator";
		...
		struct ZSlabMemoryAllocator temp;
		struct ZSlabMemoryAllocatorClass temp_class;
		...
		if (z_slab_memory_allocator_type_id == -1)
			z_slab_memory_allocator_type_id = zco_allocate_type_id();
		global->common.id = z_slab_memory_allocator_type_id;
		zco_add_to_vtable(&global->common.vtable_off_list, &global->common.vtable_off_size, z_slab_memory_allocator_type_id);
		zco_add_to_vtable(&global->common.svtable_off_list, &global->common.svtable_off_size, z_slab_memory_allocator_type_id);
		global_ptr = zco_get_ctx_type(ctx, z_slab_memory_allocator_type_id);
		*global_ptr = (ZCommonGlobal *) global;
		
		{
			...
			p_class->__dispose = z_slab_memory_allocator_dispose;
		}
		{
			...
			p_class->__allocate = z_slab_memory_allocator_allocate;
		}
		{
			ZMemoryAllocatorClass *p_class = (ZMemoryAllocatorClass *) ((char *) CLASS_FROM_GLOBAL(global) + global->common.svtable_off_list[z_memory_allocator_type_id]);
			global->__parent_allocate_aligned = p_class->__allocate_aligned;
			p_class->__allocate_aligned = z_slab_memory_allocator_allocate_aligned;
		}
		{
			ZMemoryAllocatorClass *p_class = (ZMemoryAllocatorClass *) ((char *) CLASS_FROM_GLOBAL(global) + global->common.svtable_off_list[z_memory_allocator_type_id]);
			global->__parent_try_resize = p_class->__try_resize;
			p_class->__try_resize = z_slab_memory_allocator_try_resize;
		}
		{
			ZMemoryAllocatorClass *p_class = (ZMemoryAllocatorClass *) ((char *) CLASS_FROM_GLOBAL(global) + global->common.svtable_off_list[z_memory_allocator_type_id]);
			global->__parent_resize = p_class->__resize;
			p_class->__resize = z_slab_memory_allocator_resize;
		}
		{
			ZMemoryAllocatorClass *p_class = (ZMemoryAllocatorClass *) ((char *) CLASS_FROM_GLOBAL(global) + global->common.svtable_off_list[z_memory_allocator_type_id]);
			global->__parent_deallocate = p_class->__deallocate;
			p_class->__deallocate = z_slab_memory_allocator_deallocate;
		}
		{
			ZObjectClass *p_class = (ZObjectClass *) ((char *) CLASS_FROM_GLOBAL(global) + global->common.svtable_off_list[z_object_type_id]);
			global->__parent_class_destroy = p_class->__class_destroy;
			p_class->__class_destroy = z_slab_memory_allocator_class_destroy;
		}
		__z_slab_memory_allocator_class_init(ctx, (ZSlabMemoryAllocatorClass *) CLASS_FROM_GLOBAL(global));
		...
		z_map_insert((ZMap *) global->common.method_map, strdup("new"), (ZObjectSignalHandler) new);
			...
			global_init((ZSlabMemoryAllocatorGlobal *) global);
		#endif
		return global;
	}
	return (ZSlabMemoryAllocatorGlobal *) *global_ptr;
}
...
void __z_slab_memory_allocator_init(struct zco_context_t *ctx, Self *self)
{
	struct ZSlabMemoryAllocatorGlobal *_global = z_slab_memory_allocator_get_type(ctx);
...
static void z_slab_memory_allocator_class_destroy(ZObjectGlobal *gbl)
{
	ZSlabMemoryAllocatorGlobal *_global = (ZSlabMemoryAllocatorGlobal *) gbl;
