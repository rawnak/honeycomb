...
{
{
 ...
 if (segment) {
 rc = z_vector_segment_insert(segment, iter, n, item, selfp->item_size, selfp->storage_mode,
 selfp->userdata, selfp->item_copy_construct);
 } else {
 /* If the iterator is pointing at a NULL segment but this vector is not empty, it must mean
                           that the iterator is not associated with this vector */
 assert(selfp->count == 0);
 assert(selfp->head == NULL);
 assert(selfp->tail == NULL);

 /* We need a new segment */
 segment = create_segment(self);
 z_vector_iter_set_segment(iter, segment);

 rc = z_vector_segment_insert(segment, iter, n, item, selfp->item_size, selfp->storage_mode,
 selfp->userdata, selfp->item_copy_construct);

 z_vector_segment_set_prev(segment, NULL);
 z_vector_segment_set_next(segment, NULL);
