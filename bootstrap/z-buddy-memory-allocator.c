...
#define GET_NEW(ctx,allocator) __z_buddy_memory_allocator_new(ctx,allocator)
#define INIT_EXISTS
#define init z_buddy_memory_allocator_init
#define new z_buddy_memory_allocator_new

int z_buddy_memory_allocator_type_id = -1;

static Self *__z_buddy_memory_allocator_new(struct zco_context_t *ctx, ZMemoryAllocator *allocator)
{
	Self *self = NULL;
	if (allocator) {
		ZObjectTracker *object_tracker = z_memory_allocator_get_object_tracker(allocator);
		if (object_tracker) {
			self = (Self *) z_object_tracker_create(object_tracker, z_buddy_memory_allocator_type_id);
	...
	if (!self) {
		...
		__z_buddy_memory_allocator_init(ctx, self);
	}
	return self;
}
...
static void z_buddy_memory_allocator_init(Self *self);
static void  z_buddy_memory_allocator_dispose(ZObject *object);
static void *  z_buddy_memory_allocator_allocate(ZMemoryAllocator *allocator,int size);
static void *  z_buddy_memory_allocator_allocate_aligned(ZMemoryAllocator *allocator,int size,int alignment);
static int  z_buddy_memory_allocator_get_usable_size(ZMemoryAllocator *allocator,void *block);
...
ZBuddyMemoryAllocatorGlobal * z_buddy_memory_allocator_get_type(struct zco_context_t *ctx)
{
	...
	if (z_buddy_memory_allocator_type_id != -1) {
		global_ptr = zco_get_ctx_type(ctx, z_buddy_memory_allocator_type_id);
	}
	if (!global_ptr || !*global_ptr) {
		struct ZBuddyMemoryAllocatorGlobal *global = (ZBuddyMemoryAllocatorGlobal *) malloc(sizeof(struct ZBuddyMemoryAllocatorGlobal));
		global->common.ctx = ctx;
		global->_class = malloc(sizeof(struct ZBuddyMemoryAllocatorClass));
		memset(CLASS_FROM_GLOBAL(global), 0, sizeof(struct ZBuddyMemoryAllocatorClass));
		global->common.name = "ZBuddyMemoryAllocator";
		...
		struct ZBuddyMemoryAllocator temp;
		struct ZBuddyMemoryAllocatorClass temp_class;
		...
		if (z_buddy_memory_allocator_type_id == -1)
			z_buddy_memory_allocator_type_id = zco_allocate_type_id();
		global->common.id = z_buddy_memory_allocator_type_id;
		zco_add_to_vtable(&global->common.vtable_off_list, &global->common.vtable_off_size, z_buddy_memory_allocator_type_id);
		zco_add_to_vtable(&global->common.svtable_off_list, &global->common.svtable_off_size, z_buddy_memory_allocator_type_id);
		global_ptr = zco_get_ctx_type(ctx, z_buddy_memory_allocator_type_id);
		*global_ptr = (ZCommonGlobal *) global;
		
		{
			...
			p_class->__dispose = z_buddy_memory_allocator_dispose;
		}
		{
			...
			p_class->__allocate = z_buddy_memory_allocator_allocate;
		}
		{
			ZMemoryAllocatorClass *p_class = (ZMemoryAllocatorClass *) ((char *) CLASS_FROM_GLOBAL(global) + global->common.svtable_off_list[z_memory_allocator_type_id]);
			global->__parent_allocate_aligned = p_class->__allocate_aligned;
			p_class->__allocate_aligned = z_buddy_memory_allocator_allocate_aligned;
		}
		{
			ZMemoryAllocatorClass *p_class = (ZMemoryAllocatorClass *) ((char *) CLASS_FROM_GLOBAL(global) + global->common.svtable_off_list[z_memory_allocator_type_id]);
			global->__parent_get_usable_size = p_class->__get_usable_size;
			p_class->__get_usable_size = z_buddy_memory_allocator_get_usable_size;
		}
		{
			ZMemoryAllocatorClass *p_class = (ZMemoryAllocatorClass *) ((char *) CLASS_FROM_GLOBAL(global) + global->common.svtable_off_list[z_memory_allocator_type_id]);
			global->__parent_resize = p_class->__resize;
			p_class->__resize = z_buddy_memory_allocator_resize;
		}
		{
			ZMemoryAllocatorClass *p_class = (ZMemoryAllocatorClass *) ((char *) CLASS_FROM_GLOBAL(global) + global->common.svtable_off_list[z_memory_allocator_type_id]);
			global->__parent_deallocate = p_class->__deallocate;
			p_class->__deallocate = z_buddy_memory_allocator_deallocate;
		}
		{
			ZObjectClass *p_class = (ZObjectClass *) ((char *) CLASS_FROM_GLOBAL(global) + global->common.svtable_off_list[z_object_type_id]);
		...
		z_map_insert((ZMap *) global->common.method_map, strdup("new"), (ZObjectSignalHandler) new);
			...
			global_init((ZBuddyMemoryAllocatorGlobal *) global);
		#endif
		return global;
	}
	return (ZBuddyMemoryAllocatorGlobal *) *global_ptr;
}
...
void __z_buddy_memory_allocator_init(struct zco_context_t *ctx, Self *self)
{
	struct ZBuddyMemoryAllocatorGlobal *_global = z_buddy_memory_allocator_get_type(ctx);
